...
def flatten():
    ...
    html_list = os.listdir('../reglist/templates/frb/')
    print (html_list)
    ...
    for i in files:
        if i.endswith('.txt'):
            ...
            try:
                ...
                html_name = i.replace('.txt', '_clean.html')
                if html_name in html_list:
                    path_name = '../../../files/' + i.replace('.txt', '_clean.html')
                else:
                    path_name = '../../../files/' + i.replace('.txt', '.pdf')
...
with open('../reglist/pickle/txt_names_frb.pickle', 'wb') as handle:
    pickle.dump(txt_names, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('../reglist/pickle/filenames_frb.pickle', 'wb') as handle:
    pickle.dump(filenames, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('../reglist/pickle/doc_names_frb.pickle', 'wb') as handle:
    pickle.dump(doc_names, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('../reglist/pickle/doc_contents_frb.pickle', 'wb') as handle:
...
with open('../reglist/pickle/feature_names_frb.pickle', 'wb') as handle:
    pickle.dump(feature_names, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('../reglist/pickle/tfidf_matrix_frb.pickle', 'wb') as handle:
